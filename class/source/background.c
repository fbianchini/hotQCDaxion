...
int background_ncdm_distribution(
  ...
  /** - b) deal now with case of reading analytical function */
    ...
    */
    ...
    /** Write a condition so that if n_ncdm == 0, then the distribution is a fermi-dirac with chemical potential ksi */
    if (n_ncdm == 0) {
    /**************************************************/
    ...
    else if (n_ncdm == 1) { /* axion particle */
      A = fitA(pba->m_ncdm_in_eV[1]);
      B = fitB(pba->m_ncdm_in_eV[1]);
      mu = fitmu(pba->m_ncdm_in_eV[1]);
      sign = (pba->m_ncdm_in_eV[1] <= _ma_tmp11_) ? 1 : -1; // _ma_tmp1_ is defined in background.h as 0.43 eV
      // printf("A = %e, B = %e, mu = %e, sign = %e, ma = %e\n", A, B, mu, sign, pba->m_ncdm_in_eV[1]);
      *f0 = A/(exp(q/B+mu)+sign)/pow(2*_PI_,3);
...
/** AXION PARTICLE DISTRIBUTION FUNCTION **/

double fitA(double ma) {
  if (ma < _ma_tmp_) {
    double Ap1 = 0.0006502305271540252*ma + 5.818503632670442*pow(ma, 2) + 1.7077919838609008*pow(ma, 3) 
                - 19.112702471313714*pow(ma, 4) + 217.56708968166296*pow(ma, 5) - 680.4344582884856*pow(ma, 6) 
                + 1558.1272631738277*pow(ma, 7) + 0.5883862670523259*pow(ma, 8);
    return Ap1;
  }
  else if (ma <= _ma_tmp1_) {
    double Ap2 = -1445.711960221139 + 42629.18539520443*ma - 62871.262309108024*pow(ma, 1.3213937540085885) 
                + 430923.12182241643*pow(ma, 4.113279561282635) - 3.0832955754014385e8*pow(ma, 7.5311458172894135) 
                + 1.086214809648795e9*pow(ma, 8.011651485151962) - 1.4095329235618644e9*pow(ma, 8.531145817289413) 
                + 6.446084706295363e8*pow(ma, 9.011651485151962);
    return Ap2;
  }
  else if (ma <= _ma_tmp11_) {
    double Aprange = -1.7726702231358287e8 + 6.231532627239215e8*pow(ma, 0.4) - 4.335853312684721e8*pow(ma, 0.8)
                    - 2.1550243934138918e8*ma+ 2.435055811767119e8*pow(ma, 2) + 8.117110977395008e6*pow(ma, 4.6)
                    + 1.4334949241311407e8*pow(ma, 5) - 3.02208379554428e8*pow(ma, 7.4) - 4.2015700641164e9*pow(ma, 10)
                    + 8.505355550144157e8*pow(ma, 13.4) + 6.221226040493666e10*pow(ma, 14.8)
                    - 3.4628903734736145e13*pow(ma, 26.8) + 2*pow(ma, 40.3) - 1.4174726496076028e16*pow(ma, 41.3)
                    + 1.9623167824700751e30*pow(ma, 82.6) + pow(ma, 82.85) + pow(ma, 83.1 + ma);

    return Aprange;
  }
  else if (ma <= _ma_tmp2_) {
    double  Am1 = -111815.42526378247 + 43308.133344719085*pow(-0.4484308086164486 + ma, 1 + pow(ma, 2.2871461018003068)) 
                 - 1.2707630377115556e6*ma + 497152.7925738372*pow(ma, (1/pow(ma, 0.6080797661651385))) 
                 + 1.0688102378334336e6*pow(ma, ma) - 151007.31438589358*pow(ma, (-1 + 1.4819354010983732*ma)) 
                 - 54275.75656576082*ma*pow(pow(ma, 3.1531118429733604), (1 + pow(ma, 2.228965310335438)));
    return Am1;
  }
  else if (ma <= _ma_tmp3_) {
    double Am2 = 41.356276857044755 - 105.5807337451913*ma + 89.66928979767764*pow(ma, 2.550395670716566) 
               - 847.9027376208794*pow(ma, 14.862647490675567) + 2043.2589339674944*pow(ma, 19.149036363025328);
    return Am2;
  } 
  else if (ma <= _ma_tmp4_) {
    // double Am3 = 1.1537417104131675;
    double Am3 = 32.68844888798427 - 164.37686105058444*ma + 377.1752666483464*pow(ma, 2) 
              - 488.84272872310623*pow(ma, 3) + 383.5361327353497*pow(ma, 4) - 181.39950227197113*pow(ma, 5) 
              + 47.76559605209787*pow(ma, 6) - 5.392610567703284*pow(ma, 7);
    return Am3;
  } 
  else if (ma <= _ma_tmp5_) {
    double Am4 = -7.071184335824988 + 33.08433637362032*ma - 56.61815527812428*pow(ma, 2) 
                 + 53.92887084540591*pow(ma, 3) - 31.573125941081226*pow(ma, 4) + 11.681815111649852*pow(ma, 5) 
                 - 2.672997660095277*pow(ma, 6) + 0.346249635011207*pow(ma, 7) - 0.019455334157658034*pow(ma, 8);
    return Am4;
  } 
  else {
    double Am5 = 0.9972021401912423 
                - 1.7357991199924637e7/(pow(pow(ma, 1.7053427013992555), 6))
                - (310370.4973591616*ma)/(pow(pow(ma, 1.7053427013992555), 6))
                + (31709.91625051816*pow(ma, 2))/(pow(pow(ma, 1.7053427013992555), 6))
                + 1.757840981484325e7/(pow(pow(ma, 1.7053427013992555), 5.987080063639351))
                + (2.662337672637663e19*ma)/(pow(24. + ma, 14.246745879072634))
                - (4.3320969317698063e18*pow(ma, 2))/(pow(24. + ma, 14.246745879072634))
                - 8.072037090293701e19/(pow(24.219562415956144 + ma, 14.246745879072634));
    return Am5;
}
}

double fitB(double ma) {
    float B1 = 0.8668671293274579 + 0.3291195547662697 * ma - 17.047420715344643 * pow(ma, 2) 
              + 472.8801310891604 * pow(ma, 3) - 7446.440436652806 * pow(ma, 4) + 74277.33032233003 * pow(ma, 5) 
              - 491831.4291420408 * pow(ma, 6) + 2.2106720809292286e6 * pow(ma, 7) - 6.7709104698469015e6 * pow(ma, 8) 
              + 1.3904360974510979e7 * pow(ma, 9) - 1.8298490779766582e7 * pow(ma, 10) + 1.3931996479641166e7 * pow(ma, 11) 
              - 4.660782836238877e6 * pow(ma, 12) + 1.5123231331292901 * pow(ma, 8) * log(sqrt(ma));
    float B2 = 0.8428687414492686 + 0.1813837337332452 * ma + 0.047996862929661155 * pow(ma, 2) 
               - 0.35681475079519714 * pow(ma, 3) + 0.4458217132746368 * pow(ma, 4) - 0.30385376443656803 * pow(ma, 5) 
               + 0.13073178816249206 * pow(ma, 6) - 0.037020363616507054 * pow(ma, 7) + 0.006897162858603432 * pow(ma, 8) 
               - 0.0008145125629140403 * pow(ma, 9) + 0.00005529765575388949 * pow(ma, 10) - 1.6441964422787146e-6 * pow(ma, 11);
    return (ma <= _ma_tmp11_) ? B1 : B2;
}


double fitmu(double ma) {
    double mup  = -1.687779365840429 + 2.516989503490473 * ma - 123.75610010476413 * pow(ma, 2) + 4029.7881889523255 * pow(ma, 3)
                  - 7.7844168118959535e6 * pow(ma, 6) + 3.7198766135612436e7 * pow(ma, 7) - 1.827150631568373e8 * pow(ma, 8) 
                  + 1.0579534659183358e6 * pow(ma, 9) - 4.611709613308799e8 * pow(ma, 10) + 1.8374327406107974e8 * pow(ma, 11) 
                  - 7.938951571479295e7 * pow(ma, 12.278507175355339) + 1.3369926725702998e6 * pow(ma, 4.183477006598745) * fabs(ma)
                  - 64614.54951446116 * pow(fabs(ma), 4) + 1.7075267732358408e8 * exp(ma) * pow(ma, 8.386566692252584) / cos(ma) 
                  - 1.8967236658777148e8 * pow(ma, 11.54239536478891) / cos(fabs(ma));
    double mum1 = 9841.762683707031 - 98568.93353886507 * ma + 410661.99213554343 * pow(ma, 2) - 910528.5861671113 * pow(ma, 3) 
                  + 1.1328318286128137e6 * pow(ma, 4) - 749714.2804218399 * pow(ma, 5) + 206167.53345930777 * pow(ma, 6);
    double mum2 = 15.815465356471426 - 78.53217213349245 * ma + 175.6612399648243 * pow(ma, 2) - 223.60700112182818 * pow(ma, 3) 
                  + 173.09399445980392 * pow(ma, 4) - 81.01722745234031 * pow(ma, 5) + 21.156088981634667 * pow(ma, 6) 
                  - 2.3722624547535998 * pow(ma, 7);
    double mum3 = 0.9940278587187968 - 1.270029044116554 * ma + 1.2073370731299664 * pow(ma, 3) - 1.1723670714347776 * pow(ma, 4) 
                  + 0.555708026670609 * pow(ma, 5) - 0.14785621575917426 * pow(ma, 6) + 0.021206930316054185 * pow(ma, 7) - 0.0012820282805733254 * pow(ma, 8);
    double mum4 = 0.6318177157869492 - 0.8456225855474504 * ma + 0.5435166910537697 * pow(ma, 2) - 0.20128694172656275 * pow(ma, 3) 
                  + 0.045507361895766006 * pow(ma, 4) - 0.006215077747182605 * pow(ma, 5) + 0.00047220221651496125 * pow(ma, 6)
                   - 0.000015350354500742516 * pow(ma, 7);
